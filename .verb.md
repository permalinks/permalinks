...
- [structure][]: `{String}` (required)  A template that determines what a permalink will look like when it's rendered (using the `permalink.format()` method)
- [file][]: `{String|Object}` (required) The file (object or path) for which to create the permalink
...
A permalink is created by resolving the placeholder values in a [permalink structure][] (like `:foo/:bar`) with actual data, from [locals][] and the parsed [file path][] (of the file for which we are generating a permalink):
